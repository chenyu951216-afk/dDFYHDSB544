...
from okx_scanner import scan_dual_sma_pullback_candidates
from strategy_dual_sma_pullback_2h import STRATEGY_ID, TIMEFRAME
...
@dataclass
class DualSmaPullbackStrategyConfig:
    leverage: int = 10
    td_mode: str = "cross"
    margin_ccy: str = "USDT"
...
def _compute_live_snapshot(exchange, symbol: str, candles: int) -> Dict[str, Any]:
    ...
    return {
        "symbol": symbol,
        "timeframe": TIMEFRAME,
        "current_bar_timestamp_ms": float(current[0]),
...
def manage_active_position(exchange, config: DualSmaPullbackStrategyConfig) -> Dict[str, Any]:
...
def open_new_position(exchange, config: DualSmaPullbackStrategyConfig) -> Dict[str, Any]:
    active = get_strategy_active_position(STRATEGY_ID)
    if active:
        return {"status": "blocked_active_position", "position": active}

    capital_state = get_per_strategy_allocated_equity(exchange=exchange)
    scan_result = scan_dual_sma_pullback_candidates(
        exchange=exchange,
...
def run_cycle(exchange=None, config: Optional[DualSmaPullbackStrategyConfig] = None) -> Dict[str, Any]:
    exchange = exchange or create_okx_exchange()
    config = config or DualSmaPullbackStrategyConfig()
